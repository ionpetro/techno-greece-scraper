  ...
  try {
    const browser = await puppeteer.launch(
      process.env.NODE_ENV === "production"
        ? {
            args: [ '--proxy-server=49.0.2.242:8090' ],
    ...
    await page.goto("https://ra.co/events");
    ...
    const result = await page.evaluate(() => {
      ...
      datesArray.forEach((node) => {
        ...
        eventsOfDay.forEach((eventNode) => {
          if (!eventNode) throw new Error('eventNode')
    ...
    res.status(400).json({ error: JSON.stringify(e, null, 2) });
    console.log(e);
  }
}
